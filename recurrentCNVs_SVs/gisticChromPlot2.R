#I didn't like the inbuilt function. I think all I changed was the region labelling... very closely based on the function gisticChromPlot maftools function

gisticChromPlot2 <- function (gistic = NULL, fdrCutOff = 0.1, markBands = NULL, color = NULL, 
  ref.build = "hg19", cytobandOffset = 0.01, txtSize = 0.8, 
    cytobandTxtSize = 0.6, maf = NULL, mutGenes = NULL, y_lims = NULL, 
    mutGenesTxtSize = 0.6) 
{
    ...
    if (ref.build == "hg38") {
        ...
        stop("ref.build can only be hg38")
    }
...
#Adjust y pos of cytoband peak annotation
    
    peaknames <- cyto_peaks_scores$amp *1.1
    
    #Manually jitter
    peaknames[which(cyto_peaks_scores$Cytoband=="3p21.31")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="3p21.31"),"amp"]*1.5
    peaknames[which(cyto_peaks_scores$Cytoband=="5q11.2")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="5q11.2"),"amp"]*1.4
    peaknames[which(cyto_peaks_scores$Cytoband=="5q13.2")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="5q13.2"),"amp"]*1.6
    peaknames[which(cyto_peaks_scores$Cytoband=="7p22.1")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="7p22.1"),"amp"]*1.7
    peaknames[which(cyto_peaks_scores$Cytoband=="8q11.23")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="8q11.23"),"amp"]*1.5
    peaknames[which(cyto_peaks_scores$Cytoband=="8p22")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="8p22"),"amp"]*1.3
    peaknames[which(cyto_peaks_scores$Cytoband=="11q14.1")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="11q14.1"),"amp"]*1.3
    peaknames[which(cyto_peaks_scores$Cytoband=="19q13.32")] <- cyto_peaks_scores[which(cyto_peaks_scores$Cytoband=="19q13.32"),"amp"]*1.3
    

    text(x = cyto_peaks_scores$Start_Position_updated, y = peaknames,
        labels = cyto_peaks_scores$Cytoband, font = 4, cex = txtSize)
