...
library(gtools, lib.loc="/exports/igmm/eddie/ewing-lab/software/Rlibraries_4.1/")
library(doParallel)

get_chr_sizes = function() {
    temp = read.table("../params/hg38.chrom_sizes", header = F, sep = "\t")
    chr_sizes = temp[,2]
...
centromere_telomere_coords = as.matrix(
    read.table(
        "../params/hg38_centromere_and_telomere_coords.txt",
        header = T,
        sep = "\t",
        row.names = 1
    )
...
if (length(commandArgs(T)) > 0) {
    ...
    if (!file.exists(file)) {
	    ...
	    #Remove svs with breakpoints not in hg38 chromosome sizes file from UCSC
	    autochrs <- c(as.character(1:22),"X")
	    d <- d[intersect(which(d[,1] %in% autochrs), which(d[,4] %in% autochrs)),]
