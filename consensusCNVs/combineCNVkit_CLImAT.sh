...
bedtools intersect -a ${OUTDIR}/${SAMPLE}-cnvkit-nopseudo_all_lengths.bed -b ${OUTDIR}/${SAMPLE}-CLImAT.bed -wo -f $OVERLAP  > $OUTDIR/${SAMPLE}-cnvkit-CLImAT_tmp.bed

Rscript --no-save scripts/combineCNVkit_CLImAT.R ${OUTDIR}/${SAMPLE}-cnvkit-CLImAT_tmp.bed ${OUTDIR}/${SAMPLE}-cnvkit-CLImAT_${OVERLAP}_all_lengths.bed


rm ${OUTDIR}/${SAMPLE}-cnvkit-CLImAT_tmp.bed  
