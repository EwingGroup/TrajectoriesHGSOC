...
pdf(file='coxph_combined_variables_forest_plot_20230906.pdf',width=4,height=12);
plotd <- log2(exp(cbind((confint(coxph_fit_combined))[,1],(coef(coxph_fit_combined)),(confint(coxph_fit_combined))[,2])))
...
par(cex.axis=0.7, cex.lab=1, cex.main=1.2, cex.sub=1)
plot(0,0,xlim=c(min(plotd[,1]),max(plotd[,3])*1.2),ylim=c(1,nolines),
     yaxt='n',xlab='log2(HR)',ylab='',col='white');
...
lbl[lbl=="eccDNA"] <- "EC DNA";
lbl <- gsub('_',' ',lbl);

axis(2,at=1:nolines,labels=lbl,las=2,cex=0.7)
...
pdf(file='variable_only_forest_plot_20230906.pdf',width=4,height=12);
plotd <- log2(exp(oneFeatureCoefs));
...
axis(2,at=1:nolines,labels=lbl2,las=2,cex=0.7)
legend('topleft',pch=15,col=c('red','orange','black'),
       legend=c(expression(p<=0.001),expression(p<=0.05),expression(p>0.05)),cex=0.5)
dev.off();
