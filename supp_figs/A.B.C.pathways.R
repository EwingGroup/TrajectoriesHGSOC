...
## Read in data, rename columns, and filter to pathogenic mutations (nonsense, frameshift, and missense with SIFT and PolyPhen predictions)
snvs <- fread("data/hgsocSNVs.txt.gz") %>%
  dplyr::rename("chr" = "V1", "pos" = "V2", "AF" = "V3", "GENE" = "V4", "TYPE" = "V5", 
                "IMPACT" = "V6", "BIOTYPE" = "V7", "SIFT" = "V8", "POLYPHEN" = "V9", "CANONICAL" = "V10", "SAMPLE" = "V11") %>%
  filter(GENE %in% unlist(gene_set$genesets)) %>%
  mutate(SAMPLE = gsub("-ensemble-annotated-oxog_filtered.vep107.vcf.gz", "", SAMPLE),
         SAMPLE = gsub("_2$|-P$", "", SAMPLE),
         TYPE = str_split(TYPE, "&", simplify = T)[,1],
         SIFT = gsub("\\(.*\\)", "", SIFT), 
         POLYPHEN = gsub("\\(.*\\)", "", POLYPHEN)) %>%
  filter(IMPACT == "HIGH" | (SIFT == "deleterious" & POLYPHEN == "probably_damaging")) %>%
  filter(TYPE != "inframe_deletion" | TYPE != "inframe_insertion") %>%
  filter(GENE != "." & !grepl("^MT-", GENE)) %>%
...
for(i in unique(consolidated$SAMPLE)){
  
  message(i)
  single <- consolidated %>% filter(SAMPLE == i)
...
# SV Pathways #
###############
## Load in pathogenic structural variants
...
## label these as "multi-hit"
consolidated <- long %>%
  filter(CATEGORY == "SV" & GENE %in% unlist(gene_set$genesets)) %>%
  dplyr::select(SAMPLE, GENE, MUT_TYPE) %>%
  group_by(GENE, SAMPLE) %>%
  dplyr::mutate(MUT_TYPE = ifelse(length(SAMPLE) > 1, "multi_hit", MUT_TYPE)) %>%
  distinct() 
...
for(i in unique(consolidated$SAMPLE)){
  ...
  for(x in 1:length(gene_set$genesets)){
    ...
    value <- ifelse(nrow(single_pathway) == 0, "", 
                    ifelse(nrow(single_pathway) == 1, single_pathway$MUT_TYPE,
                           ifelse(length(unique(single_pathway$MUT_TYPE)) == 1, paste0("multi_gene_", unique(single_pathway$MUT_TYPE)), "multi_gene")))
...
## label these as "multi-hit"
consolidated <- long %>%
  filter(CATEGORY == "CNV" & GENE %in% unlist(gene_set$genesets)) %>%
  dplyr::select(SAMPLE, GENE, MUT_TYPE) %>%
  group_by(GENE, SAMPLE) %>%
  dplyr::mutate(MUT_TYPE = ifelse(length(SAMPLE) > 1, "multi_hit", MUT_TYPE)) %>%
  distinct() 
...
col = c("multi_gene_CNV_DUP" = "#009E734D", 
        "CNV_DUP" = "#009E73",
        "multi_gene_CNV_DEL" = "#0072B24D",
        "multi_gene" = "#330072B2", 
        "CNV_DEL" = "#0072B2")
...
heatmap_legend_param = list(title = "Alterations",
                            ...
                            labels = c("Multi-Gene-Duplication", "Duplication", "Multi-Gene-Deletion", "Multi-Gene Hit", "Deletion"))
